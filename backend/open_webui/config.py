...
frontend_favicon = FRONTEND_BUILD_DIR / "static" / "aethyron.svg"

if frontend_favicon.exists():
    try:
        shutil.copyfile(frontend_favicon, STATIC_DIR / "aethyron.svg")
    except Exception as e:
        logging.error(f"An error occurred: {e}")
        ...
        if r.ok:
            if "logo" in data:
                ...
                if r.status_code == 200:
                    with open(f"{STATIC_DIR}/aethyron.svg", "wb") as f:
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (Aegis + Ollama)
...
DEFAULT_ARENA_MODEL = {
    ...
    "name": "Arena Model",
    "meta": {
        "profile_image_url": "/aethyron.svg",
